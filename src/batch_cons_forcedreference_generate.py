...
current_model = str(sys.argv[1])
current_condition = int(sys.argv[2])
...
models = [
    ...
    ("facebook/xglm-7.5B", 2, 0, None, True), # The larger models should be ran on a bigger GPU
    # ("malteos/bloom-350m-german", 32, 0, None, False),
    # ("malteos/bloom-1b5-clp-german", 4, 0, None, False),
    ("malteos/bloom-6b4-clp-german", 2, 0, None, False) # The larger models should be ran on a bigger GPU
]
...
for model_name, batch_size, device, device_map, is_sentencepiece in models:
    ...
    exp3.to_csv(f"../data/cons_forced_coreference--{model_name.replace('/', '--')}--condition-{current_condition}.csv", sep=";", index=False)
